...
\FIXME{Methods are described in details here}

\FIXME{General introduction: Scenario, unbinned Likelihood, signal and background contributions. Possible connection to multimessenger recap. Sample description and characterization in analysis part.}
...
\TODO{show or ref to plot of sigma vs kappa; Maybe explain both PDFs more?}.
...
In the following we adapt to the standard notation and use $n$ instead of $\lambda$ for the Poisson expectations.
...
\label{equ:TS}
\end{equation}
\TODO{Make notation consistent, use correct background per source.}
