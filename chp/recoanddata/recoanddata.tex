...
To approximately and conservatively account for unknown systematics, the transformed maps are smeared with a symmetric Gaussian kernel of width $\SI{1}{\sigma}$ as mentioned before.
...
\end{equation}
over the unit sphere and setting the resulting map to zero outside the $\SI{6}{\sigma}$ level.
Due to the lack of a proper test statistic, the Likelihood value $\mathcal{L}_\text{cut}$ for the $\SI{6}{\sigma}$ level can only be estimated from Wilks' theorem with
\begin{equation}
  \mathcal{L}_\text{cut} =
